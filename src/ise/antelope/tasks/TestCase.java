...
public class TestCase extends Task implements TestStatisticAccumulator {
     ...
     * containing the name of the failed target and the reason why it failed.
     */
    ...
    /**
     * @param b if true, show the summary output (number of tests, passed, failed)
     * after the completion of all tests.  Optional, default is true, do show
        * summary.
     */
    ...
    /**
     * @param f the file containing the tests to execute. Required.  The file itself
     * is a standard Ant build file, but will be treated differently than if Ant itself
     * ran it.  If there is a target named "setUp", that target will be executed first,
     * then all targets with names starting with "test" (not in any particular order),
     * then if there is a target named "tearDown", that target will be executed last.
     * All other targets are ignored.
     */
    ...
    /**
     * Should Asserts be enabled?  Many (most?) tests will use the Assert task, which
     * requires a property to be set to actually enable the asserts.  By default, 
     * Asserts are enabled for testcases.
     * @param b if false, do not enable asserts.  Note that this sets an Ant property,
        * and due to property immutability, this attribute may have no effect if it has
        * been set already.  Generally, asserts are enabled or disabled for an entire build.
     */
    ...
    /**
     * Should the build fail if the test fails?  By default, a failed test does not
    ...
    /**
     * @return the name of the test module, this is the project name if there is one,
     * otherwise, the filename of the test file.
     */
     ...
     * groups of tests in a 'suite' by setting a single property.  Optional, default
        * is true, the test should run.
     */
     ...
     * omitting those that have already been set in the project as
